...
function generateRefferralCode(username) {
    const randomString = Math.random().toString(36).substring(2, 8); // Generate a random string
    return `${username}-${randomString}`; // Combine username with the random string
}
...
async function signUp(req, res, next) {
    try {
        const data = await req.userData;
        if (data) {
            return res.status(409).json({ message: "Another user is still logged in" });
        }
        ...
        bcrypt.hash(req.body.password, bcrypt.genSaltSync(10), null, async (err, hash) => {
            if (err) {
            ...
            try {
                ...
                const userPayload = {
                    ...
                    referralCode: generateRefferralCode(req.body.username),
                    password: hash,
                ...
                await models.Activity.findOrCreate({
                    where: { type: 'register', user_id: newUser.id },
                    defaults: { message: 'registered' }
                });
...
async function show(req, res) {
    ...
    if (data) {
        ...
        const totalArticles = await models.Post.count({
            where: { userId: id }, // adjust if your foreign key is different
        });
        models.User.findByPk(id, {
            include: [
